...
async def amireallyalive(event):
    ...
    start = datetime.now()
    jmthonevent = await edit_or_reply(event, "ㅤ")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  - "
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس النجار يعمل بنجاح**"
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس النجار يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار النجار :** `{jmthonversion}`\n"
...
ROZ_PIC = "https://telegra.ph/file/d0ec503a68a905a799936.jpg"
RAZAN = Config.TG_BOT_USERNAME
ROZ_T = (
    f"**⌯︙بوت النــجار يعمل بنجاح 🤍،**\n"
    f"**   - اصدار التليثون :** `1.23.0\n`"
    f"**   - اصدار النــجار :** `4.0.0`\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jmub.uid:
            buttons = [
                [
                    Button.url("قنـاة السـورس", "https://t.me/hasoni_lq"),
                    Button.url("المطـور", "https://t.me/hsshh"),
                ]
            ]
